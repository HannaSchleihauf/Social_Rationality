...
keep_cond <- c("Subject No Evidence", "Subject Evidence")
...
boot.main_exp1$ci.predicted <- boot.main_exp1$ci.predicted %>%
  mutate(
    ...
    show_alpha = ifelse(Condition %in% c("Subject No Evidence", "Subject Evidence"), 1, 0)
  )
...
plot_Condition <-
  ...
  geom_point(
    ...
    aes(
      ...
      color = Condition, alpha = show_alpha
    ), size = 3.5
  ) +
  scale_x_discrete(
  ...
  geom_violinhalf(
    data = xxdata.agg.Chimp,
    aes(x = Condition, y = mean.resp, fill = Condition, alpha = show_alpha),
